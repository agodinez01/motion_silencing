...
if setting.TEST ~= 0  % use the mouse instead of the eye tracker
    ...
    if setting.eyeUsed == 0 || setting.eyeUsed == 1 
        % get latest float sample
        evt = Eyelink('NewestFloatSample');
        t_pc = GetSecs;
        x   = evt.gx(setting.eyeUsed+1);
        y   = evt.gy(setting.eyeUsed+1);
        t_eye   = evt.time;
        pa  = evt.pa(setting.eyeUsed+1);
    ...
    % first find eye that's being tracked
        ...
        setting.eyeUsed = Eyelink('EyeAvailable'); % get eye that's tracked
        if setting.eyeUsed == el.BINOCULAR % if both eyes are tracked
            setting.eyeUsed = 0; % use the first eye as default
        end
